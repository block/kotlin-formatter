...
@Service
class FormatScriptStateService() {
  //: FormatScriptStateService {
  //: SimplePersistentStateComponent<PathState>(PathState()) {

  var path: String = "bin/format"

  fun locateScriptFile(project: Project) {
    //path = "bin/kotlin-format"
    ...
    //val unzipDir = File("kotlin-formatter-binary").also {
      if (!it.exists()) {
        it.mkdirs()
      }
    }
    ...
    val zipFile = File(project.basePath, "kotlin-formatter-dist.zip")
      //File("kotlin-formatter-dist.zip")
    //File(project.basePath, "kotlin-formatter-dist.zip")
